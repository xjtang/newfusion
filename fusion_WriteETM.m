    ...
    for I_Day=1:numel(main.date.swath)
        ...
        for I_TIME = 1:numel(File.MOD09SUB)
            ...
            if main.set.dif == 0
                ...
                [~,ETMImage(:,:,3),~] = swath2etm(MOD09SUB.DIF09RED500,MOD09SUB,ETMGeo,500);
                [~,ETMImage(:,:,4),~] = swath2etm(MOD09SUB.DIF09NIR500,MOD09SUB,ETMGeo,500);
                ...
                [~,ETMImage(:,:,7),~] = swath2etm(MOD09SUB.DIF09RED250,MOD09SUB,ETMGeo,250);
                [~,ETMImage(:,:,8),~] = swath2etm(MOD09SUB.DIF09NIR250,MOD09SUB,ETMGeo,250);
                [~,ETMImage(:,:,9),~] = swath2etm(MOD09SUB.QACloud250,MOD09SUB,ETMGeo,250);
                [~,ETMImage(:,:,10),~] = swath2etm(MOD09SUB.QACloud500,MOD09SUB,ETMGeo,500);
            else
                [~,~,ETMImage(:,:,1)] = swath2etm(MOD09SUB.DIF09BLU500,MOD09SUB,ETMGeo,500);
                [~,~,ETMImage(:,:,2)] = swath2etm(MOD09SUB.DIF09GRE500,MOD09SUB,ETMGeo,500);
                [~,~,ETMImage(:,:,3)] = swath2etm(MOD09SUB.DIF09RED500,MOD09SUB,ETMGeo,500);
