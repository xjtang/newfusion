...
function fusion_WriteCSV(main)
    ...
    for I_Day = 1:numel(main.date.swath)
        ...
        % find files
        File.MOD09SUB = dir([FileName.MOD09SUB,'M*D','09SUB','*',DayStr,'*']);
        ...
        for I_TIME = 1:numel(File.MOD09SUB)
            ...
            % get data for csv
            [header, csv] = swath2csv(MOD09SUB,250,main.etm.utm);

            % write csv
            outFile = [main.output.csv,thisPlat,'09CSV.','250m.',DayStr,'.',TimeStr,'.csv'];
            f = fopen(outFile,'wt');
            fprintf(f, [strjoin(header,',') '\n']);
            fclose(f);
            dlmwrite(outFile,csv,'-append','delimiter',',','precision',10);
