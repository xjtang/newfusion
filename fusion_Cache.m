...
function fusion_Cache(main)
    ...
    % get dates of images and export as a seprate file
    TSDate = ones(numel(fusImage),2);
    for i = 1:numel(fusImage)
        TSDate(i,1) = str2double(fusImage(i).name(10:16));
        TSDate(i,2) = str2double(fusImage(i).name(18:21));
    end
    File.Check = dir([main.output.cache 'ts.r' i '*' '.mat']);
    if numel(File.Check) >= 1
        disp('Date list already exist, overwrite.');
    end
    TSDate = int32(TSDate);
    save([],'TSDate');
