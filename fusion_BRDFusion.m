...
%
% Updates of Version 6.3.4 - 12/12/2015
...
function fusion_BRDFusion(main)
    ...
    for I_Day = 1:numel(main.date.swath)
        ...
        % find MOD09SUB files
        File.MOD09SUB = dir([main.output.modsub,'M*D','09SUB','*',DayStr,'*']);
        ...
        File.Check = dir([main.output.modsubbrdf 'M*D' '*' 'ALL' '*' DayStr '*']);
        ...
        % find ETM BRDF files
        File.ETMBRDF = dir([main.output.etmBRDF,'ETM','M*D','BRDF_A',DayStr,'*.hdr']);
        ...
        ETMBRDF = ETMBRDF/main.cons.mcdsf;
        ...
        for I_TIME = (numel(File.Check)+1):numel(File.MOD09SUB)
            ...
            output = dir([main.output.modsubbrdf thisPlat '*' 'ALL' '*' DayStr '.' TimeStr '*']);
            if numel(output)>=1
            ...
            % save
            save([main.output.modsubbrdf,thisPlat,'09FUS.','ALL.',DayStr,'.',TimeStr,'.mat'],'-struct','MOD09SUB');
