...
%
% Updates of Version 6.3.3 - 12/2/2015
...
function fusion_WriteHDF(main)
    ...
    for I_Day = 1:numel(main.date.swath)
        ...
        % find files
        File.MOD09SUB = dir([FileName.MOD09SUB,plat,'09FUS','*',DayStr,'*']);
        ...
        for I_TIME = 1:numel(File.MOD09SUB)
            ...
            system(['cp ' main.input.swath thisPlat '*' DayStr '*' TimeStr '* ' main.output.fusion]);
            
            % load MOD09SUBBRDF
            BRDFlag = main.set.brdf;
            if BRDFlag == 1
                File.MOD09SUBB = dir([main.output.modsubbrdf,thisPlat,'09FUS.','ALL.',DayStr,'.',TimeStr,'.mat']);
                if numel(File.MOD09SUB) < 1
